...
def main(params,dataset,alg_name):
  ...
  for img in dp.iterImages(split = 'test', max_images = max_images):
    ...
    print 'Video: ' + img_blob['img_path'][0].split('/')[7]
  ...
  res_file_name = 'data/'+dataset+'/results/captions_val_'+alg_name+'_results.json'
  json.dump(captions_res, open(res_file_name, 'w'))

def run(checkpoint,dataset,alg_name):
  parser = argparse.ArgumentParser()
  parser.add_argument('--checkpoint_path', type=str, default=checkpoint, help='the input checkpoint')
