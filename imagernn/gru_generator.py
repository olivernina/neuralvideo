...
class GRUGenerator:
  ...
  def forward(Xi, Xs, model, params, **kwargs):
    ...
    Hinz = np.zeros((n, WLSTM.shape[0])) # xt, ht-1, bias
    Hout = np.zeros((n, d))
    IFOG = np.zeros((n, d * 4))
    ...
    for t in xrange(n):
      ...
      Hinz[t] = Hin[t]
      Hinz[t,1+d:] = Hinz[t,1+d:]*  IFOGf[t,d:2*d]
      IFOG[t,3*d:] = Hinz[t].dot(WLSTM[:,3*d:])
  ...
  def backward(dY, cache):
    ...
    for t in reversed(xrange(n)):
      ...
      Hinz[t] = Hin[t]
      IFOG[t,d:2*d] = dIFOG[t,3*d:]*Hinz[t].dot(WLSTM[:,3*d:])
  ...
  def predict(Xi, model, Ws, params, **kwargs):
    ...
    def LSTMtick(x, h_prev, c_prev):
      ...
      Hinz = np.zeros((1,WLSTM.shape[0])) # xt, ht-1, bias
      Hin[t,0] = 1
      Hin[t,1:1+d] = x
      Hin[t,1+d:] = h_prev
