...
class BCLSTMGenerator:
  """
  A multimodal Coupled long short-term memory (CLSTM) generator
  """
  ...
  def init(input_size, hidden_size, output_size):
    ...
    model['WLSTM'] = initw(input_size + hidden_size + 1, 4 * hidden_size) / np.sqrt(input_size + hidden_size)
  ...
  def forward(Xi, Xs, model, params, **kwargs):
    ...
    for t in xrange(n):
      ...
      if t > 0: C[t] += C[t-1] -(IFOGf[t,:d]*C[t-1])
      if tanhC_version:
  ...
  def backward(dY, cache):
    ...
    for t in reversed(xrange(n)):
      ...
      if tanhC_version:
        ...
        dIFOGf[t,2*d:3*d] = tanhCt * dHout[t]   #dht/dWo = tanh(ct) delta^l+1  #Output gate
      ...
      if t > 0:
        # dIFOGf[t,d:2*d] = C[t-1] * dC[t] # Forget gate
        # dC[t-1] += IFOGf[t,d:2*d] * dC[t]
        dC[t-1] += (1-IFOGf[t,:d]) * dC[t]


      # dIFOGf[t,:d] = IFOGf[t, 3*d:] * dC[t] # Input gate
      dIFOGf[t,:d] = -C[t] * dC[t] # Input gate
      # dIFOGf[t, 3*d:] = IFOGf[t,:d] * dC[t] # Gating function
      dIFOGf[t, 3*d:] = dC[t] # Gating function
      ...
      # y = IFOGf[t,:d]
      # dIFOG[t,:d] = (y*(1.0-y)) * dIFOGf[t,:d]  #all three sigmoid functions
      #
      # y = IFOGf[t,2*d:3*d]
      # dIFOG[t,2*d:3*d] = (y*(1.0-y)) * dIFOGf[t,2*d:3*d]  #all three sigmoid functions

      y = IFOGf[t,:3*d]
      dIFOG[t,:3*d] = (y*(1.0-y)) * dIFOGf[t,:3*d]  #all three sigmoid functions
