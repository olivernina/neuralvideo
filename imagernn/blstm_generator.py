...
class BLSTMGenerator:
  """ 
  A multimodal Batched long short-term memory (LSTM) generator
  """
  ...
  #################################################################
    ...
    #   k=3
    #   (y1, h, c) = LSTMtick(Ws[ixprev], h, c)
    #   top_idxs,top_probs = get_top(y1,k)
    #
    #   tprob = np.zeros(len(top_idxs))
    #   tidxs = [[]for i in range(len(top_idxs))]
    #   ths = [[]for i in range(len(top_idxs))]
    #   tcs = [[]for i in range(len(top_idxs))]
    #
    #   for i,iy in enumerate(top_idxs):
    #     tidxs[i].append(iy)
    #     probs,idxs,hs,cs = traverse(Ws,iy,y1,h,c,nsteps,k)
    #     tprob[i] = top_probs[i] + probs
    #     tidxs[i].extend(idxs)
    #     ths[i] = hs
    #     tcs[i] = cs
    #
    #
    #   top_idx = tprob.argmax()
    #   predix.extend(tidxs[top_idx])#,tprob[tprob.argmax()]
    #   ixprev = predix[-1]
    #   h = ths[top_idx]
    #   c = tcs[top_idx]
    #   nsteps=k
