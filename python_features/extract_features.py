...
samples_per_video = int(args.spv)
...
if samples_per_video > 1:
    ...
    for indx in range(0,len(allftrs),samples_per_video):
        mean_ftrs.append(numpy.mean(allftrs[indx:indx+samples_per_video],0))
    scipy.io.savemat(os.path.join(base_dir, args.out), mdict =  {'feats': np.transpose(mean_ftrs)})
elif samples_per_video ==1:
    scipy.io.savemat(os.path.join(base_dir, args.out), mdict =  {'feats': np.transpose(allftrs)})
